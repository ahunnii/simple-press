...
async function handle(request: Request) {
  ...
  try {
    ...
    // 3. Publish scheduled pages/blog posts. Pages without a publishedAt get
    ...
    //    product/variant is purchasable again. Semantics mirror the
    //    storefront `isInStock` helper (src/hooks/use-shop-filters.ts) plus
    //    the checkout validation's `comingSoon` guard. Per-request
    //    try/catch — a failed send leaves notifiedAt null so it retries on
    //    the next run. Requests whose product/variant no longer exists are
    //    retired (notifiedAt set) without an email. Requests for a business
    //    with the backInStock feature flag disabled are skipped (not
    //    retired) so they resume automatically if the flag is re-enabled.
    ...
    // 5. Archive past events (admin Upcoming/Past split only — never
    //    load-bearing for hiding a past event from the storefront).
    //    Platform-wide, not feature-flag-gated; see archivePastEvents'
    //    docblock for why.
    ...
    //    gating and per-source error isolation live inside
    //    `syncVideoSources`.
    //
    //    This endpoint ticks every ~15 minutes, but a source is only
    //    eligible once its lastSyncedAt is older than MIN_SYNC_INTERVAL_MS
    //    (30 min). That guard is what stops us re-fetching every registered
    //    feed on every tick and hammering YouTube — so most ticks are a
    //    single cheap SELECT.
    //
    ...
    if (monitoringEnabled && checkInId) {
      ...
      // Six results.X assignments above (staleReservations,
      // scheduledProducts, scheduledPages, backInStock, archivePastEvents,
      // videoSync) run unconditionally in sequence, so a complete run always
      // populates exactly 6 entries — hardcoded rather than derived because
      // there's no single source of list to derive it from without
      // restructuring the job list itself. If a 7th job is ever added here,
      // bump this too.
      const allRan = jobs.length === 6;
