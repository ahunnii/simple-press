...
export function NoiseCollectionPage({
  ...
  const products = collection.collectionProducts
    .map((cp) => cp.product)
    .filter((p): p is NonNullable<typeof p> => p != null) as unknown as Product[];
  ...
  return (
    <PageTransition>
      ...
      <section
        className="border-b-2 border-foreground px-6 pt-12 pb-14 text-center"
                    ...
                    >
                      ...
                      >
                        <svg
                          ...
                          aria-hidden="true"
                        >
                          <path d={getCategorySilhouette(col.name)} fill="var(--vn-bone)" opacity="0.9" />
                        </svg>
                      </div>
                      <div className="vn-cat-meta flex flex-col gap-1 px-3.5 py-3.5">
                        <span className="vn-cat-num font-mono text-[10px] tracking-[0.22em] uppercase" style={{ color: "var(--vn-steel)" }}>
                          {String(i + 1).padStart(2, "0")}
                        </span>
                        <span className="vn-cat-name font-serif italic leading-[1.15]" style={{ fontSize: "clamp(1.1rem, 1.6vw, 1.6rem)", letterSpacing: "-0.01em", color: "var(--vn-ink)" }}>
                          {col.name}
                        </span>
                        <span className="vn-cat-count font-mono text-[10px] tracking-[0.18em] uppercase" style={{ color: "var(--vn-ink-soft)" }}>
