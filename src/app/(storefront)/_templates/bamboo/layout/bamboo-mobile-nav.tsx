 ...
 * that would leave the `.bamboo` root and with it every token this markup reads.
 *
...
export function BambooMobileNav({
  ...
  // Scoped to the `.bamboo` root's own children rather than a document-wide
  // `main`/`footer` lookup — the editor preview can mount a second storefront.
  useEffect(() => {
    if (!open) return;
    const root = dialogRef.current?.closest(".bamboo");
    if (!root) return;
