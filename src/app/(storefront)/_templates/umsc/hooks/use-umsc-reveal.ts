...
/**
 * Lightweight IntersectionObserver-based scroll-reveal hook — copy of vii's
 * `useViiReveal` (`../../vii/hooks/use-vii-reveal.ts`), rescoped to `.umsc`.
 *
 ...
 */
export function useUmscReveal(threshold = 0.1): {
  ...
  useEffect(() => {
    // Arm the reveal system: the `.umsc-js` flag gates the hidden state in
    ...
    el?.closest(".umsc")?.classList.add("umsc-js");
    ...
    const io = new IntersectionObserver(
      ...
      // scrolls fully into view — reads as smoother.
