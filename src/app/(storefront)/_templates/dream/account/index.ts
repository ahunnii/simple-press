import type { TemplateField, TemplateFieldGroup } from "~/lib/template-fields";
import type { TemplateSection } from "~/lib/template-sections";

/**
 * Account pages (Settings, Security, Address Book, Orders, Order Detail,
 * Preferences, Subscriptions) are thin dream skins over shared/better-auth-ui
 * account UI and DB-driven order/subscription data — none of it is
 * owner-editable template content. Per the "Account pages" playbook entry
 * ("no template fields for any of these pages... None of these have template
 * fields, so none are reachable in /editor") and `wealth/account/index.ts`'s
 * own precedent, this exports empty arrays only so Phase 4's aggregation
 * stays uniform across every domain.
 */

export const dreamAccountData: TemplateField[] = [];

export const dreamAccountFieldGroups: TemplateFieldGroup[] = [];

export const dreamAccountSections: TemplateSection[] = [];
