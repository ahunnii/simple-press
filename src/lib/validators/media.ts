...
/**
 * S3 object key validator — closes a path-traversal hole where a key like
 * `bizA/../bizB/file.jpg` passes the router's `key.startsWith(`${businessId}/`)`
 * prefix check (it DOES start with `bizA/`) but resolves, once `..` segments
 * are normalized (`new URL()`, which aws4fetch calls before signing), to an
 * object under a different tenant's prefix — turning a scoped delete/presign
 * into a cross-tenant one.
 *
 * Deliberately a DENYLIST of dangerous patterns rather than a strict
 * positive-character allowlist: real keys are generated by `uniqueKey()`
 * (`src/app/api/upload/route.ts`) and `contentAddressedKey()`
 * (`src/lib/s3/put.ts`) as `{businessId}/{prefix}-{hex}{ext}` or
 * `{businessId}/testimonials/{hex}{ext}` — always plain ASCII with no spaces
 * or unicode — but a positive regex risks rejecting some legitimately
 * generated or hand-uploaded key shape we didn't anticipate. The router's own
 * `key.startsWith(...)` prefix checks stay in place as defense in depth; this
 * schema only rules out the shapes that can escape that prefix or smuggle
 * control bytes through to S3/aws4fetch.
 */
export const storageKeySchema = z
  ...
  .refine((key) => !key.includes("\\"), {
    message: "Key must not contain backslashes",
  })
  .refine((key) => !key.includes("%"), {
    message: "Key must not contain percent-encoded characters",
  })
  .refine((key) => !key.startsWith("/"), {
    message: "Key must not start with a slash",
  })
  .refine((key) => !/[\x00-\x1f\x7f]/.test(key), {
    message: "Key must not contain control characters",
  })
  .refine((key) => !key.includes("//"), {
    message: "Key must not contain empty path segments",
  })
  .refine(
    (key) =>
      key !== ".." &&
      ...
      !key.includes("/..") &&
      !key.split("/").includes(".."),
    { message: "Key must not contain a parent-directory segment" },
  );
